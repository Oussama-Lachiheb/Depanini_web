...
class PromotionController extends AbstractController
{
    ...
    {
        $donnees = $promotionRepository->findAll();
    ...
    #[Route('/cd', name: 'app_promotion_indexx', methods: ['GET'])]
    public function indexx(PromotionRepository $promotionRepository,PaginatorInterface $paginator,Request $request): Response
    {
        $donnees = $promotionRepository->findAll();
        $articles = $paginator->paginate(
            $donnees,
        ...
        return $this->render('promotion/indexx.html.twig', [
            'promotions' =>$articles,
        ]);
    }

    #[Route('/new', name: 'app_promotion_new', methods: ['GET', 'POST'])]
    public function new(Request $request,\Swift_Mailer $mailer, PromotionRepository $promotionRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $start = $form->get('dateDebuPromo')->getData()->format('d-m-Y');
            $end=$form->get('dateFinPromo')->getData()->format('d-m-Y');

            $message = (new \Swift_Message('Promotion'))
                ->setFrom('mohamed.dhokkar@esprit.tn')
                ->setTo('ayoub.hamzaoui@esprit.tn')
                ->setBody(
                    $this->renderView('mail/promotion_mail.html.twig',[
                        'start'=>$start,
                        'end'=>$end
                    ]),
                    'text/html'
    ...
    #[Route('/{idPromo}', name: 'app_promotion_show', methods: ['GET'])]
    ...
    #[Route('/{idPromo}/edit', name: 'app_promotion_edit', methods: ['GET', 'POST'])]
    ...
    #[Route('/{idPromo}', name: 'app_promotion_delete', methods: ['POST'])]
